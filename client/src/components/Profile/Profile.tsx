...
export const Profile = () => {
    ...
    const [placement, setPlacement] = useState<'bottom' | 'left'>('left');
    ...
    const onFileChangeMainPhoto = (e: React.ChangeEvent<HTMLInputElement>, popupState: any) => {
        if (e?.target?.files?.[0]) {
            ...
            if (file.size > MAX_FILE_SIZE) {
                ...
                setTimeout(() => setPhotoSizeError(false), 3000);
            } else {
    ...
    return (
        <form onSubmit={onSubmitHandler} className="profile">
            <AlertComponent isError={deletePhotoSuccess} text={'Фото успешно удалено!'} status={'success'} />
            <AlertComponent isError={editLawyerSuccess} text={'Измененные данные вступят в силу после проверки модерации!'} status={'success'} />
            <AlertComponent isError={photoSizeError} text={'Размер фото превышает 2 MB'} status={'error'} />
                                                    ...
                                                    src={
                                                        info.photo
                                                            ? changeMainPhotoLocally
                                                                ? changeMainPhotoLocally
                                                                : `/api/uploads/${info.photo}`
                                                            : `/static/no-image.png`
                                                    }
                                                    alt="profile-photo"
                                                />
                                                <img className="profile__info-hiddenImage" src={`/static/camera.png`} alt="camera-img" />
                                            </div>
                                        </Button>
                                ...
                                <div className="profile__types">
                                    <div className="profile__types-inner">
                                        <div className="profile__types-lawyer">
                                            <p>
                                                {t('Cabinet.activity')}:&nbsp;
                                                {Array.isArray(info.caseCategories) && info.caseCategories.length > 0 ? (
                                                    storedLanguage === 'ru' ? (
                                                        info.caseCategories
                                                            .map(value => abc.find(category => category.id === value)?.ru)
                                                            .filter(Boolean)
                                                            .join(', ')
                                                    ) : (
                                                        info.caseCategories
                                                            .map(value => abc.find(category => category.id === value)?.kz)
                                                            .filter(Boolean)
                                                            .join(', ')
                                                    )
                                                ) : (
                                                    <h1>{info.caseCategories && 'Пусто'}</h1>
                                                )}
                                            </p>
                                    ...
                                    <div className="profile__docs-bg">
                                        ...
                                        {previews.length > 0
                                            ? previews.map((item, index) => (
                                                  <PreviewImg key={index} src={item.src} onDelete={() => onDeleteDoc(index)} name="" />
                                              ))
                                            : t('Cabinet.absent')}

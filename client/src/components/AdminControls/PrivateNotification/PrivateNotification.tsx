...
export const AdminPrivateNotification = () => {
    ...
    const { user, questionId, questionText } = location.state || {};
    ...
    return (
        ...
        {(() => {
            const { color, text } = getRole(user.role);
            return (
                <Typography variant="h6" align="center"  sx={{color}}>
                    {t('Panel.private.recipient')}: {text} {user.name + " " + user.surname }
                </Typography>
        );})()}
        {questionText && (
        ...
        <TextField label="ссылка на источник (не обязательно)" type="text" value={sourceLink} onChange={e => setSource(e.target.value)} fullWidth />

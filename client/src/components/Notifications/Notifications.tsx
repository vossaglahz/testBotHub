...
export const Notifications = () => {
    ...
    const { data: generalNotifications, isLoading: isLoadingGeneral, refetch: refetchGeneral } = useGetAllGeneralNotificationsQuery({
        currentPage,
        important,
        sorted,
    });
    const { data: personalNotifications, isLoading: isLoadingPersonal, refetch: refetchPersonal } = useGetAllPersonalNotificationsQuery({
        currentPage,
        sorted,
    });
    const [isMobile, setIsMobile] = useState<boolean>(window.innerWidth <= 767);
    const { user, lawyer } = useAppSelector(state => state.users);
    const [generalNotif, setGeneral] = useState<number[]>([...JSON.parse(user.viewedNotifications || lawyer.viewedNotifications || '[]')]);

    const refetchNotifications = isPrivate === 'general' 
    ? () => { refetchGeneral() }
    : () => { refetchPersonal() };
    ...
    }, [isPrivate, refetchGeneral, refetchPersonal]);
    ...
    const addToGeneralNotif = (id: number) => {
                ...
                {notifications.notifications.length > 0 ? (
                    notifications.notifications.map(item => (
                        <Notification
                            ...
                            isViewed={item.isViewed || undefined}
                            viewedNotify={generalNotif}
                            setNotify={addToGeneralNotif}
                            refetched={refetchNotifications}
                        />
                    ))
                ) : (

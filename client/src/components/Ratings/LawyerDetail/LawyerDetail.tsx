...
export const LawyerDetail = () => {
    ...
    console.log('categories: ', lawyer?.caseCategories);
    ...
    const translateCategories = (categories: string[]): string => {
        return categories.map(category => categoryTranslations[category] || category).join(' ');
    };
                ...
                {lawyer ? (
                    <div>
                        <div className="image__full-name">
                            <div>
                                <img
                                    src={lawyer.photo ? BASE_URL + lawyer.photo : `/static/no-image.png`}
                                    alt={lawyer.name}
                            ...
                            <div className="lawyer__data">
                                <p>
                                    {t('Aside.ratings.nameDetails')}: {lawyer.name}
                                </p>
                                <p>
                                    {t('Aside.ratings.surnameDetails')}: {lawyer.surname}
                                </p>
                                {lawyer.patronymicName && (
                                    <p>
                                        {t('Aside.ratings.patrNameDetails')}: {lawyer.patronymicName}
                                    </p>
                                )}
                                <p>
                                    {t('Aside.ratings.type')}: {lawyer.lawyerType || t('Aside.ratings.typeAbsent')}
                                </p>
                                <p>
                                    {t('Aside.ratings.category')}:{' '}
                                    {categories.length > 0 ? translateCategories(categories) : t('Aside.ratings.categoryAbsent')}
                                </p>

                                <p>
                                    {t('Aside.ratings.city')}: {lawyer.city || t('Aside.ratings.typeAbsent')}
                                </p>
                            </div>
                        </div>
                            ...
                            {lawyer?.documents?.[0]?.image && lawyer.documents[0].image.length > 0
                                ? lawyer.documents[0].image.map((imgSrc, index) => (
                                      <div className="previewImg" key={index} title={imgSrc.name}>
                                          ...
                                          <a
                                              className="previewImg__name"
                                              target="_blank"
                                              href={`${BASE_URL}${imgSrc.src}`}
                                        ...
                                        {[5, 4, 3, 2, 1].map(stars => {
                                            ...
                                            return (
                                                <div key={stars} className="rating-row">
                                                    ...
                                                    <p>{percentage}%</p>
                                                    <p>
                                                        {count} {t('Aside.ratings.reviewes')}
                                                    </p>
                                                </div>
                                            );
                                        })}
                                    ...
                                    ratings.map(item => (
                                        <div className="user__review-wrap" key={item.id}>
                                            <div className="user__review-photo">
                                                <img src={item.user.photo ? BASE_URL + item.user.photo : `/static/no-image.png`} alt="lawyer-photo" />

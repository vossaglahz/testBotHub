...
import { IBlockUser, ISubscribeLawyer, IUser, UserRoles } from '@/interfaces/IUser.inerface';
...
export class UserRepo {
    ...
    async registration(userDto: RegistrationUserDto | RegistrationLawyerDto) {
        try {
            ...
            if (userDto.role === 'user' || userDto.role === 'admin') {
                ...
                const foundUserLawyer = await this.lawyerRepo.findOne({ where: { email: userDto.email } });
                if (foundUserLawyer) throw new Error(`Пользователь с почтовым адресом ${userDto.email} уже существует`);
                ...
                await mailerSend.sendActivationMail(userDto.email, `${process.env.VITE_API_URL}/api/users/activate/${activationLink}`);
                ...
                if (foundUser) throw new Error(`Пользователь с почтовым адресом ${userDto.email} уже существует`);
                const foundUserPerson = await this.repo.findOne({ where: { email: userDto.email } });
                if (foundUserPerson) throw new Error(`Пользователь с почтовым адресом ${userDto.email} уже существует`);
    ...
    async login(loginUserDto: SignInDto) {
        try {
            ...
            if (user) {
                const blockedDate = lawyerDate.toLocaleString();
                if (date.getTime() < lawyerDate.getTime()) throw new Error(`Пользователь заблокирован временно до ${blockedDate}`);
            ...
            if (!user) {
                ...
                if (date.getTime() < lawyerDate.getTime()) throw new Error(`Пользователь заблокирован временно до ${blockedDate}`);
                if (lawyer.permanentBlocked) throw new Error(`Пользователь заблокирован навсегда`);
    ...
    async getLawyersList(filters: { page?: number; limit?: number; lawyerId?: number }) {
        ...
        try {
            if (filters.lawyerId) {
                const lawyer = await this.lawyerRepo
                    .createQueryBuilder('lawyer')
                    .leftJoinAndSelect('lawyer.rating', 'rating')
                    .leftJoinAndSelect('rating.user', 'user')
                    .leftJoinAndSelect('lawyer.documents', 'documents', 'documents.publish = :publish', { publish: true })
                    .where('lawyer.id = :id', { id: filters.lawyerId })
                ...
                const averageRating =
                    lawyer.rating.length > 0 ? lawyer.rating.reduce((sum, { assessment }) => sum + assessment, 0) / lawyer.rating.length : 0;
    ...
    async deletePhoto(refreshToken: string) {
        ...
        if (user?.role === 'user') {
            await this.repo.save({ ...user, photo: '' } as User);
        }

        if (user?.role === 'lawyer') {
            await this.lawyerRepo.save({ ...user, photo: '' } as Lawyer);
        }
    }
    ...
    async subscribeLawyer(data: ISubscribeLawyer) {
        const { id, dateSubscription } = data;
        try {
            const lawyer = await this.lawyerRepo.findOne({ where: { id } });
            if (!lawyer) return { success: false, message: 'Юрист не найден' };
            if (!dateSubscription) return { success: false, message: 'Некорректная дата' };
            const currentDate = new Date();
            const subscriptionDate = new Date(dateSubscription);

            if (subscriptionDate < currentDate) {
                lawyer.dateSubscription = null;
            } else {
                lawyer.dateSubscription = subscriptionDate.toISOString();
            }
            await this.lawyerRepo.save(lawyer);
            return { success: true, message: 'Юрист получил подписку успешно' };
        } catch (error) {
            console.error(error);
            return { success: false, message: error instanceof Error ? error.message : 'Неизвестная ошибка' };
        }
    ...
    async sendMail(refreshToken: string): Promise<void> {
        let client = await this.findByRefreshToken(refreshToken);

        if (!client) {
            throw new Error('Invalid activation link ');
        }
        await this.repo.findOne({
            where: { refreshToken: client.refreshToken },
        });
        const mailerSend = new Mailer();
        const activationLink = `${process.env.VITE_API_URL}/api/users/activate/${client.activationLink}`;

        try {
            await mailerSend.sendActivationMail(client.email, activationLink);
        } catch (error) {

...
import { SubscribeLawyerDto } from '@/dto/subscribeLawyer.dto';
...
export class UserController {
    ...
    activate: RequestHandler = async (req, res): Promise<void> => {
        try {
            ...
            return res.redirect(`${process.env.VITE_API_URL}`);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    login: RequestHandler = async (req, res): Promise<void> => {
        try {
            ...
            } else if (error.message.includes('Пользователь заблокирован временно до')) {
                const blockedUntil = error.message.split('до')[1];
                res.status(400).json({ message: `Пользователь заблокирован временно до ${blockedUntil}` });
            } else {
    ...
    deletePhoto: RequestHandler = async (req, res): Promise<void> => {
        try {
            const { refreshToken } = req.cookies;
            const request = await this.service.deletePhoto(refreshToken);
            res.status(200).send(request);
        } catch (error) {
    ...
    subscribeLawyer: RequestHandler = async (req, res): Promise<void> => {
        try {
            const subscribeDto = plainToInstance(SubscribeLawyerDto, req.body);
            const errors = await validate(subscribeDto);
            if (errors.length > 0) {
                const validationMessages = errors
                    .map(err => {
                        return Object.values(err.constraints || {});
                ...
                const request = await this.service.subscribeLawyer(req.body);
                if (request.success) {
                    res.status(200).send(request);
                } else {
                    res.status(400).send(request);
    ...
    sendEmail: RequestHandler = async (req, res): Promise<void> => {
        try {
            const { refreshToken } = req.cookies;
            ...
            await this.service.sendEmail(refreshToken);

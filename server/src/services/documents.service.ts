...
export class DocsService {
    ...
    publishDocs = async (docId: number, lawyerId: number) => {
        return await this.repository.publishDocs(docId, lawyerId);
    };
    ...
    async rejectDoc(refreshToken: string, requestId: string) {
        try {
            const request = await this.repository.rejectDoc(refreshToken, requestId);
            if (!request) {

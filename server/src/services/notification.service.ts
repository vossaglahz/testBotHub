...
export class NotificationService {
    ...
    async markAsViewedPersonal(notificationId: number) {
        return await this.repository.markPersonalNotifications(notificationId);
    }
    ...
    async markAsViewedGeneral(notificationId: number, refreshToken: string) {
        ...
        let response;
        if (user.role === 'user') {
            response = await this.repository.markNotificationAsViewedForUser(notificationId, refreshToken);
        } else if(user.role === 'lawyer') {
            response = await this.repository.markNotificationAsViewedForLawyer(notificationId, refreshToken);
        }
        return response;
    }
}

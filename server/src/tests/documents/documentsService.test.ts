...
describe('DocsService', () => {
    ...
    describe('publishDocs', () => {
        it('успешная публикация документа', async () => {
            ...
            const result = await docsService.publishDocs(mackId, lawyerId);

            expect(mockRepository.publishDocs).toHaveBeenCalledWith(mackId, lawyerId);
    ...
    describe('rejectDoc', () => {
        it('успешное отклонение документа', async () => {
            ...
            const result = await docsService.rejectDoc(refreshToken, requestId);

            expect(mockRepository.rejectDoc).toHaveBeenCalledWith(refreshToken, requestId);
        ...
        it('возвращает ошибку, если возникает ошибка в репозитории', async () => {
            ...
            await expect(docsService.rejectDoc(refreshToken, requestId)).rejects.toThrow('Ошибка при удалении документа');
            expect(mockRepository.rejectDoc).toHaveBeenCalledWith(refreshToken, requestId);
        });
        ...
        it('возвращает общую ошибку при неизвестной ошибке', async () => {
            ...
            await expect(docsService.rejectDoc(refreshToken, requestId)).rejects.toThrow('Ошибка отклонения запроса');
            expect(mockRepository.rejectDoc).toHaveBeenCalledWith(refreshToken, requestId);
        });
    });
});
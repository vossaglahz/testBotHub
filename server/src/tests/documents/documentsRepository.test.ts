...
describe('DocsRepository', () => {
    ...
    describe('publishDocs', () => {
        it('успешная публикация документа', async () => {
            ...
            const result = await docsRepository.publishDocs(docId, lawyerId);
    ...
    //         (mockEntityManager.findOne as jest.Mock).mockResolvedValue(id);
    //         (mockEntityManager.delete as jest.Mock).mockResolvedValue(newDoc.id);

    //         const result = await docsRepository.rejectDoc(refreshToken, requestId);
    //         expect(mockEntityManager.findOne).toHaveBeenCalledWith({ where: { id } });
    //         expect(mockEntityManager.delete).toHaveBeenCalledWith(id);

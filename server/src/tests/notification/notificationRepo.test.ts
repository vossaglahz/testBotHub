...
describe('NotificationRepo', () => {
    ...
    describe('postQuestion', () => {
        it('should save a personal notification from a user', async () => {
            ...
            expect(result).toEqual({ message: 'Ваш ответ успешно отправлен' });
            expect(repo['personalNotification'].save).toHaveBeenCalledWith(
    ...
    describe('markNotificationAsViewedForUser', () => {
        it('should mark notification as viewed for a user', async () => {
            const refreshToken = 'mockToken';
            const user = { id: 1, viewedNotifications: '[]', role: 'user' };
            const notificationId = 1;
            ...
            expect(user.viewedNotifications).toEqual(JSON.stringify([notificationId]));
            expect(userRepo.saveUser).toHaveBeenCalledWith(user, user.role);
        });
        ...
        it('should throw an error if user is not found', async () => {
            ...
            await expect(repo.markNotificationAsViewedForUser(1, refreshToken)).rejects.toThrow('Пользователь не найден');
        });
    });
});

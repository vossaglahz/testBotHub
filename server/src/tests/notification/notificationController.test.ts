...
describe('NotificationController', () => {
    ...
    beforeAll(async () => {
        ...
        const userMock: RegistrationUserDto = {
            role: 'user',
        ...
        userMock.password = await hashPassword(userMock.password);
        userMock.activationLink = uuidv4();
        userAccessToken = (await generateAccessToken(userMock)).accessToken;
        userRefreshToken = (await generateRefreshToken(userMock)).refreshToken;
        userMock.accessToken = accessToken;
        userMock.refreshToken = refreshToken;
    });
    afterAll(async () => {
    ...
    describe('POST /notifications/personal', () => {
    it('должен вернуть 200 и сообщение об успешной отправке вопроса', async () => {
        ...
        (NotificationService.prototype.postQuestion as jest.Mock).mockResolvedValue({ message: 'Ваш вопрос успешно отправлен админу' });
    ...
    it('должен вернуть 400 при ошибке валидации', async () => {
        ...
        jest.spyOn(NotificationService.prototype, 'postQuestion').mockResolvedValue({
